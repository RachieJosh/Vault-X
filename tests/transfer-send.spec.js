...
test.describe('Transfer - Send', () => {
  ...
  test('TC_SEND-001 - Verify send tab loads correctly', async ({ page }) => {
    await expect(page).toHaveURL(/.*transfer/);
  });

  test('TC_SEND-002 - Verify asset selector is visible', async () => {
    await expect(send.assetSelector).toBeVisible();
  });

  test('TC_SEND-003 - Verify amount input, address input and submit button are visible', async () => {
